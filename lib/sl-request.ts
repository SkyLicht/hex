...
// Updated response handler that extracts data from the standard response format
export async function responseHandler<T = never>(response: Response, url: string): Promise<T> {
    ...
    if (jsonResponse.status >= 400) {
        throw new ServerUnreachableError(jsonResponse.message || "API returned error status");
    }
...
export async function responseHandlerFull<T = never>(response: Response, url: string): Promise<ApiResponse<T>> {
    // 1) Handle specific status codes first
    if (response.status === 401) {
        throw new HTTP_401_Error("You are not logged in!");
    }
