...
class RendererMachineFT extends RendererMachineContainer {
    constructor(
        x: number,
        y: number,
        viewBox: ViewBoxType,
        ...
        if (variant === 'ft_grid_machine') {
            switch (viewBox) {
                case ViewBoxType.H_W_M_1536:
                    return { width: 250, height: 70 } // Loader-specific dimensions
                case ViewBoxType.H_W_L_1080:
                    return { width: 40, height: 40 } // Larger loader for bigger viewport
                default:
...
export default RendererMachineFT
