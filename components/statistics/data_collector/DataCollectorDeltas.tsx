...
const PPIDDeltasDisplay = ({ groupName, lineName }: Props) => {
    const [activeTab, setActiveTab] = useState<
        'statistics' | 'deltas' | 'grouped'
    >('statistics')

    console.log(groupName, lineName)
    const { data, loading, error, refetch } = usePPIDCurrentDayDeltas(
        groupName,
        lineName,
        {
                        ...
                        {activeTab === 'statistics' && (
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                ...
                                <div className="bg-gray-50 p-4 rounded-lg">
                                    ...
                                    <p className="text-2xl font-bold text-gray-900">
                                        {formatDelta(
                                            data.statistics.avg_delta_seconds
                                        )}
                                    </p>
                                </div>
                                <div className="bg-gray-50 p-4 rounded-lg">
                                    ...
                                    <p className="text-2xl font-bold text-green-600">
                                        {formatDelta(
                                            data.statistics.min_delta_seconds
                                        )}
                                    </p>
                                </div>
                                <div className="bg-gray-50 p-4 rounded-lg">
                                    ...
                                    <p className="text-2xl font-bold text-red-600">
                                        {formatDelta(
                                            data.statistics.max_delta_seconds
                                        )}
                                    </p>
                                </div>
                                <div className="bg-gray-50 p-4 rounded-lg">
                                ...
                                {data.listed_deltas.map((delta, index) => (
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-2">
                                            ...
                                            <span
                                                className={`inline-flex px-3 py-1 text-sm font-semibold rounded-full ${getDeltaColor(delta.delta_seconds)}`}
                                            >
                                                {formatDelta(
                                                    delta.delta_seconds
                                                )}
                        ...
                        {activeTab === 'grouped' && (
                            <div>
                                <h3 className="text-lg font-medium mb-4">
                                    Deltas Under 1 Minute (
                                    {data.grouped_deltas.deltas_1_min.length})
                                </h3>
                                <div className="overflow-hidden">
                                    <table className="min-w-full bg-white border border-gray-200 rounded-lg shadow">
                                        ...
                                        <tbody className="bg-white divide-y divide-gray-200">
                                            {data.grouped_deltas.deltas_1_min.map(
                                                (delta, index) => (
                                                    <tr
                                                        key={index}
                                                        className={
                                                    ...
                                                    >
                                                        ...
                                                        <td className="px-4 py-4 text-sm text-center border-b">
                                                            <span
                                                                className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getDeltaColor(delta.delta_seconds)}`}
                                                            >
                                                                {formatDelta(
                                                                    delta.delta_seconds
                                                                )}
